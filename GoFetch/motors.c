...
static int32_t steps_to_complete_right_motor = 0;
static int32_t steps_to_complete_left_motor = 0;
static uint8_t ongoing = 0;
...
static void right_motor_timer_callback(PWMDriver *gptp)
{
   (void) gptp;
   uint8_t i;
   if ((right_motor.direction == BACKWARD) && ((right_motor.count>steps_to_complete_right_motor)||ongoing)) {
       i = (right_motor.step_index + 1) & 7;
       right_motor.update(step_table[i]);
       right_motor.count -= 1;
       right_motor.step_index = i;
   } else if ((right_motor.direction == FORWARD) && ((right_motor.count<steps_to_complete_right_motor)||ongoing)) {
       i = (right_motor.step_index - 1) & 7;
       right_motor.update(step_table[i]);
       right_motor.count += 1;
       right_motor.step_index = i;
   } else if (right_motor.count==steps_to_complete_right_motor){
...
static void left_motor_timer_callback(PWMDriver *gptp)
{
   (void) gptp;
   uint8_t i;
   if ((left_motor.direction == BACKWARD) && ((left_motor.count>steps_to_complete_left_motor)||ongoing)) { // Inverted for the two motors
       i = (left_motor.step_index + 1) & 7;
       left_motor.update(step_table[i]);
       left_motor.count += 1;
       left_motor.step_index = i;
   } else if ((left_motor.direction) == BACKWARD && ((left_motor.count<steps_to_complete_left_motor)||ongoing)) {
       i = (left_motor.step_index - 1) & 7;
       left_motor.update(step_table[i]);
       left_motor.count -= 1;
       left_motor.step_index = i;
   } else if(left_motor.count==steps_to_complete_left_motor) {
...
void left_motor_set_steps_to_complete(int steps) {
    steps_to_complete_left_motor = steps;
}

void right_motor_set_steps_to_complete(int steps) {
    steps_to_complete_right_motor = steps;
}
