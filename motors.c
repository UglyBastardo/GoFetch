...
//========================================
int32_t left_count_limit=0;//changer le zero en infini?
int32_t right_count_limit=0;
uint8_t ongoing = 0;
...
static void right_motor_timer_callback(PWMDriver *gptp)
{
    (void) gptp;
    uint8_t i;
    if ((right_motor.direction == BACKWARD) && (right_motor.count > right_count_limit) && (ongoing==1)) {
        i = (right_motor.step_index + 1) & 7;
        right_motor.update(step_table[i]);
        right_motor.count -= 1;
        right_motor.step_index = i;
    } else if ((right_motor.direction == FORWARD) && (right_motor.count < right_count_limit) && (ongoing==1)) {
        i = (right_motor.step_index - 1) & 7;
        right_motor.update(step_table[i]);
        right_motor.count += 1;
        right_motor.step_index = i;
...
static void left_motor_timer_callback(PWMDriver *gptp)
{
    (void) gptp;
    uint8_t i;
    if ((left_motor.direction == FORWARD) && (left_motor.count < left_count_limit) && (ongoing==1)) { // Inverted for the two motors
        i = (left_motor.step_index + 1) & 7;
        left_motor.update(step_table[i]);
        left_motor.count += 1;
        left_motor.step_index = i;
    } else if ((left_motor.direction == BACKWARD) && (left_motor.count > left_count_limit) && (ongoing==1)) {
        i = (left_motor.step_index - 1) & 7;
        left_motor.update(step_table[i]);
        left_motor.count -= 1;
        left_motor.step_index = i;
    ...
    //==================================================
    /*if (left_motor.count >= left_count_limit){
    	left_count_limit=INFINITY;
    	left_motor.update(step_halt);
